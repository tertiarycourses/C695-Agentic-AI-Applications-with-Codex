...
**Version:** v1.0
**Individual folder:** `labs/lab-08-headless-exec/`
...
`Define task → Choose JSON → Run exec → Parse events → Check exit → Archive evidence`

## Expected output

CI-safe command wrapper and JSONL evidence
...
1. Open `lab-08-headless-exec/` and read this guide and `evidence/checklist.md` before starting.
...
5. Give Codex the bounded task in `prompt.md`: Run the supplied repair task non-interactively. Emit JSON events, preserve stderr, and return non-zero if the required tests do not pass.
...
10. Compare the result with the acceptance rule: The wrapper avoids embedded secrets, records structured events and distinguishes success from failure by exit status.
