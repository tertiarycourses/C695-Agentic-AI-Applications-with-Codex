...
**Version:** v1.0
**Individual folder:** `labs/lab-05-parallel-worktrees/`
...
`Find seams → Create branches → Create worktrees → Run tasks → Test commits → Integrate`

## Expected output

Isolation map, two commits and integration evidence
...
1. Open `lab-05-parallel-worktrees/` and read this guide and `evidence/checklist.md` before starting.
...
5. Give Codex the bounded task in `prompt.md`: Deliver documentation and test-fixture changes in separate worktrees. Do not edit the same file. Return commit SHAs and integration order.
...
10. Compare the result with the acceptance rule: The worktrees use distinct paths and branches, produce separate commits, and integrate with the full test suite passing.
