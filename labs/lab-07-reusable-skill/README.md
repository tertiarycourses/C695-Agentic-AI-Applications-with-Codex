...
**Version:** v1.0
**Individual folder:** `labs/lab-07-reusable-skill/`
...
`Define trigger → Write skill → Add template → Run positive → Run near-miss → Refine`

## Expected output

Skill package and trigger evaluation report
...
1. Open `lab-07-reusable-skill/` and read this guide and `evidence/checklist.md` before starting.
...
5. Give Codex the bounded task in `prompt.md`: Create a narrowly triggered skill for Python unittest failure triage. Include evidence requirements and test one prompt that must not trigger it.
...
10. Compare the result with the acceptance rule: The skill passes structural checks and its evaluation shows correct trigger and near-miss behavior.
...
The skill passes structural checks and its evaluation shows correct trigger and near-miss behavior.
