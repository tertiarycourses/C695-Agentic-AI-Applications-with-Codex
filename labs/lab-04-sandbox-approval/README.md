...
**Version:** v1.0
**Individual folder:** `labs/lab-04-sandbox-approval/`
...
`Classify action → Set sandbox → Run probe → Observe block → Request approval → Recommend policy`

## Expected output

Permission experiment log and recommended policy
...
1. Open `lab-04-sandbox-approval/` and read this guide and `evidence/checklist.md` before starting.
...
5. Give Codex the bounded task in `prompt.md`: Probe only the supplied lab paths. Do not alter files outside the lab. Explain every boundary and record whether the action was allowed, blocked or approved.
...
10. Compare the result with the acceptance rule: The log contains safe probes for three permission cases and a least-privilege recommendation supported by results.
...
The log contains safe probes for three permission cases and a least-privilege recommendation supported by results.
