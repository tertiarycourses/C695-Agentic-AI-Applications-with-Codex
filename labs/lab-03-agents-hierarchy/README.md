...
**Version:** v1.0
**Individual folder:** `labs/lab-03-agents-hierarchy/`
...
`Read root → Read nested → Resolve scope → Execute task → Inspect diff → Record trace`

## Expected output

Two-level instruction hierarchy and compliance trace
...
1. Open `lab-03-agents-hierarchy/` and read this guide and `evidence/checklist.md` before starting.
...
5. Give Codex the bounded task in `prompt.md`: Modify the API fixture under its nearest instructions. Explain which rules apply before editing and verify compliance after the patch.
...
10. Compare the result with the acceptance rule: The trace distinguishes root and nested rules and the patch complies with both in the correct precedence.
