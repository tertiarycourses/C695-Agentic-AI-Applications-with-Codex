...
**Version:** v1.0
**Individual folder:** `labs/lab-02-prompt-plan-feature/`
...
`State goal → Inspect code → Write plan → Apply patch → Run checks → Review diff`

## Expected output

Minimal patch, passing tests and decision summary
...
1. Open `lab-02-prompt-plan-feature/` and read this guide and `evidence/checklist.md` before starting.
...
5. Give Codex the bounded task in `prompt.md`: Add priority filtering to TaskBoard. Preserve the CLI contract, add tests, and finish only when the focused and regression checks pass.
...
10. Compare the result with the acceptance rule: The diff is scoped, the new behavior is tested, all supplied tests pass and the final summary cites evidence.
...
The diff is scoped, the new behavior is tested, all supplied tests pass and the final summary cites evidence.
