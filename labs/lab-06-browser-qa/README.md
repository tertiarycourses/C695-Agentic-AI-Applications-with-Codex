...
**Version:** v1.0
**Individual folder:** `labs/lab-06-browser-qa/`
...
`Serve app → Open page → Exercise flow → Capture defect → Repair UI → Re-test`

## Expected output

Browser trace, screenshot and verified repair
...
1. Open `lab-06-browser-qa/` and read this guide and `evidence/checklist.md` before starting.
...
5. Give Codex the bounded task in `prompt.md`: Test the priority filter at desktop and mobile viewports. Capture the failing state, repair only the causal UI layer, and repeat the path.
...
10. Compare the result with the acceptance rule: The evidence includes pre-repair failure, scoped patch and passing desktop/mobile re-test.
...
The evidence includes pre-repair failure, scoped patch and passing desktop/mobile re-test.
